...
function entryOf(official, nonOfficialByProvider, amount, currency) {
	...
	return {
		...
		buckets: all,
		official: { ...official },
		nonOfficialByProvider: nonOfficialByProvider.map((item) => ({
...
function priceSamples(samples, prices, config, modelOnly, providerBaseUrls) {
	...
	const nonOfficialByProvider = /* @__PURE__ */ new Map();
	let amount = 0;
	let currency = "CNY";
	for (const sample of samples) if (isOfficialProvider(sample.provider, providerBaseUrls)) {
		...
		amount += costOf(sample.buckets, tier === void 0 ? void 0 : periodPricesOf(tier, sample.time ?? Date.now(), config));
	} else {
	...
	return {
		...
		currency,
		official,
		nonOfficialByProvider: [...nonOfficialByProvider.entries()].map(([provider, buckets]) => ({
			provider,
			buckets
		}))
	};
...
async function computeCosts(session, config, currentCwd, providerBaseUrls = {}) {
	...
	const lastTurnPriced = maxTurn >= 0 ? priceSamples([...samples.values()].filter((s) => s.turn === maxTurn), config.tiers, config, sessionModel, providerBaseUrls) : {
		amount: 0,
		currency: "CNY",
		official: zeroBuckets(),
		nonOfficialByProvider: []
	};
...
function parseTodayFile(path, isZstd, todayStart, config, providerBaseUrls) {
	...
	const sample = {
		...
		nonOfficialByProvider: /* @__PURE__ */ new Map()
	};
	let currentModel;
	let currentProvider;
	let first = true;
	for (const line of text.split("\n")) {
		...
		if (isOfficialProvider(currentProvider, providerBaseUrls)) {
			...
			const providerPair = sample.nonOfficialByProvider.get(key) ?? {
				peak: zeroBuckets(),
				offPeak: zeroBuckets()
			};
...
function costByModel(byModel, config) {
	...
	for (const [model, pair] of byModel) {
		...
		if (tier !== void 0) currency = tier.currency;
		const now = Date.now();
		amount += costOf(pair.officialPeak, tier === void 0 ? void 0 : periodPricesOf(tier, now, config));
		amount += costOf(pair.officialOffPeak, tier === void 0 ? void 0 : tier.offPeak);
	}
	return {
		amount,
		currency
...
async function scanToday(currentCwd, config, providerBaseUrls) {
	...
	const projectByModel = /* @__PURE__ */ new Map();
	const allByModel = /* @__PURE__ */ new Map();
	const projectOfficialTotal = {
		peak: zeroBuckets(),
		offPeak: zeroBuckets()
	};
	const allOfficialTotal = {
		peak: zeroBuckets(),
		offPeak: zeroBuckets()
	};
	const projectNonOfficial = /* @__PURE__ */ new Map();
	const allNonOfficial = /* @__PURE__ */ new Map();
			...
			for (const candidate of candidates) {
				...
				const foldProvider = (targetOfficial, targetNonOfficial) => {
					...
					for (const [provider, providerPair] of sample.nonOfficialByProvider) {
						const t = targetNonOfficial.get(provider) ?? {
							peak: zeroBuckets(),
							offPeak: zeroBuckets()
						};
...
const name = "dsh-balance-by-token";
const inject = [
	"shell",
	"settings",
	"commands"
