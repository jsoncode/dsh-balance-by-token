window.__ModuleLoader__.load({ id: 'dsh-balance-by-token', factory: (require) => {
...
/**
* dsh-balance-by-token —— 浏览器半边：样式注入（与 dsh-jenkins 相同的 bundle CSS 注入模式）。
*/
const CSS_ID = "dsh-balance-by-token/settings.css";
const css = [
	...
	".dshb-footer-right{display:flex;flex-direction:column;align-items:flex-start;gap:1px;line-height:1.15;min-width:0}",
	".dshb-footer-balance{font-size:12px;font-weight:500;color:var(--dsw-alias-label-primary,#222)}",
	...
	".dshb-prov-side{flex:none;text-align:right;min-width:150px}",
	".dshb-prov-amount{font-size:15px;font-weight:600}",
	...
	".dshb-official-title{margin-top:10px;font-size:11px;color:var(--dsw-alias-label-secondary,#888)}",
	".dshb-nonofficial-bar{margin-top:10px;padding-top:8px;border-top:1px dashed var(--dsw-alias-border-l1,#e5e5e5)}",
	".dshb-nonofficial-title{font-size:11px;color:var(--dsw-alias-label-secondary,#888)}",
	".dshb-provider-row{display:flex;align-items:center;gap:8px;margin-top:6px;font-size:11px;flex-wrap:wrap}",
	".dshb-provider-name{white-space:nowrap;font-weight:500;color:var(--dsw-alias-label-primary,#222);min-width:64px}",
	".dshb-provider-counts{white-space:nowrap;color:var(--dsw-alias-label-secondary,#888)}",
	...
	".dshb-price-cat{width:52px;text-align:center;font-size:12px;color:var(--dsw-alias-label-secondary,#666);white-space:nowrap}",
	".dshb-price-cat sup{font-size:9px;color:var(--dsw-alias-label-tertiary,#999)}",
...
function injectStyles() {
	if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
		...
		tag.dataset.plugin = "dsh-balance-by-token";
...
/**
* dsh-balance-by-token —— 浏览器半边：语言与文案（中英双语，跟随主界面语言）。
*/
...
const COPY = {
	zh: {
		...
		tokensOutput: "输出",
		costHint: "金额仅按 DeepSeek 官方（api.deepseek.com）请求计费；所有服务商的 token 用量均统计数量。宿主缓存 60 秒；今日两项扫描 ~/.dsh/sessions 日志。",
		officialLabel: "官方（api.deepseek.com）",
		nonOfficialLabel: "非官方（不计费）",
		...
		confirmRestore: "将丢弃当前价格配置（档位与时段窗口）并恢复 DeepSeek 官方默认，确认？",
		pricesSaved: "价格已保存",
	...
	en: {
		...
		tokensOutput: "Output",
		costHint: "Only DeepSeek official (api.deepseek.com) requests are billed; token usage from all providers is counted. Host caches 60s; today entries scan ~/.dsh/sessions logs.",
		officialLabel: "Official (api.deepseek.com)",
		nonOfficialLabel: "Non-official (not billed)",
		...
		confirmRestore: "Discard the current price config (tiers and windows) and restore DeepSeek official defaults?",
...
/**
* dsh-balance-by-token —— 浏览器半边：与宿主通信。
*
...
/**
* dsh-balance-by-token —— 浏览器半边：统一「余额」弹框开关（footer 入口 ↔ overlay 弹框共享）。
*/
...
function FooterButton({ onOpen, reportSession, wide = false, useSessions, run, useOpen }) {
	...
	const fullLabel = t("balanceBtn") + balText + (peak === null ? "" : peak ? t("btnPeak") : t("btnOffPeak"));
		...
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("img", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					className: "dshb-footer-right",
					children: [balText !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
...
function HeaderButton({ sessionId, run, useTick }) {
	...
	const [balText, setBalText] = (0, react.useState)("--");
	const refresh = (0, react.useCallback)(async () => {
		try {
		...
		try {
			...
			const info = (Array.isArray(list) ? list.find((b) => b.ok === true && Array.isArray(b.balance_infos) && b.balance_infos.length > 0) : void 0)?.balance_infos?.[0];
			if (info !== void 0) setBalText(info.total_balance);
		} catch {}
	}, [run, sessionId]);
	(0, react.useEffect)(() => {
		refresh();
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("headerBtnMid") }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				className: "dshb-header-amount",
				children: [balText, " CNY"]
			})
		]
	});
}
...
function BalanceModal({ run, useOpen, close, getSession, useTick, useAutoSeconds, setAutoSeconds }) {
	...
	const [defaults, setDefaults] = (0, react.useState)([]);
	const [windowCfg, setWindowCfg] = (0, react.useState)({
		timezoneOffsetMinutes: 480,
		peakWindows: []
	});
	const [defaultWindow, setDefaultWindow] = (0, react.useState)({
		timezoneOffsetMinutes: 480,
		peakWindows: []
	});
	...
	const loadPrices = (0, react.useCallback)(async () => {
		...
		if (res.ok) {
			...
			if (Array.isArray(def?.tiers)) setDefaults(def.tiers);
	...
	const restoreDefaults = () => {
		...
		setWindowCfg({
			timezoneOffsetMinutes: defaultWindow.timezoneOffsetMinutes,
			peakWindows: defaultWindow.peakWindows.map((w) => ({ ...w }))
		});
		savePrices(clone, {
			timezoneOffsetMinutes: defaultWindow.timezoneOffsetMinutes,
			peakWindows: defaultWindow.peakWindows.map((w) => ({ ...w }))
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: b === void 0 ? balLoading ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							className: "dshb-prov-sub",
							children: "—"
						}) : infos.map((info, i) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dshb-prov-amount",
							children: [info.total_balance, /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								className: "dshb-prov-sub",
	...
	const costCard = (label, entry) => {
		...
		const nonOfficialList = entry?.nonOfficialByProvider ?? [];
		return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: fmtTokens(official[seg.key]) })] }, seg.key))
				}),
				nonOfficialList.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "dshb-nonofficial-bar",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dshb-nonofficial-title",
						children: t("nonOfficialLabel")
					}), nonOfficialList.map((item) => {
						const itemTotal = totalTokensView(item.buckets);
						if (itemTotal <= 0) return null;
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dshb-provider-row",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dshb-provider-name",
									children: item.provider
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dshb-bar dshb-bar-sm dshb-bar-flex",
									role: "img",
									"aria-label": TOKEN_SEGMENTS.filter((seg) => item.buckets[seg.key] > 0).map((seg) => t(seg.labelKey) + ": " + fmtTokens(item.buckets[seg.key])).join(", "),
									children: TOKEN_SEGMENTS.map((seg) => {
										const v = item.buckets[seg.key];
										return v > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											className: "dshb-bar-seg",
											style: {
												width: v / itemTotal * 100 + "%",
												background: seg.color
											},
											title: t(seg.labelKey) + ": " + fmtTokens(v)
										}, seg.key) : null;
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dshb-provider-counts",
									children: TOKEN_SEGMENTS.filter((seg) => item.buckets[seg.key] > 0).map((seg) => t(seg.labelKey) + " " + fmtTokens(item.buckets[seg.key])).join(" · ")
								})
							]
						}, item.provider);
					})]
	...
	const renderPricesTab = () => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
		...
		prices === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { className: "dshb-spinner" }) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("table", {
				...
				}, tier.id))] }) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tbody", { children: METRIC_GROUPS.map((group, gi) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("tr", { children: [
					gi === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("th", {
						rowSpan: METRIC_GROUPS.length * 2,
						className: "dshb-price-cat",
						children: [t("priceCategory"), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("sup", { children: "(1)(2)" })]
					}) : null,
...
/**
* dsh-balance-by-token —— 浏览器半边插件主体（slots 注册）。
*
...
const FOOTER_ENTRY_ID = "dsh-balance-by-token";
function createPlugin() {
	return {
		name: "dsh-balance-by-token",
		inject: [
			"slots",
			"remote",
			"remote.commands",
...
/**
* dsh-balance-by-token —— 浏览器半边入口（tsdown 打包）。
*
