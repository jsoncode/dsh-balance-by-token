...
export function BalanceModal({ run, useOpen, close, getSession, useTick, useAutoSeconds, setAutoSeconds }) {
    ...
    const [defaultWindow, setDefaultWindow] = useState({ timezoneOffsetMinutes: 480, peakWindows: [] });
    ...
    const loadPrices = useCallback(async () => {
        ...
        if (res.ok) {
            ...
            if (Array.isArray(def?.tiers))
                setDefaults(def.tiers);
            if (config !== undefined) {
                setWindowCfg({
                            ...
                            return (_jsxs("div", { className: "dshb-prov", children: [_jsxs("div", { className: "dshb-prov-main", children: [_jsxs("div", { className: "dshb-prov-name-row", children: [_jsx("span", { className: "dshb-prov-name", children: p.label }), _jsx("span", { className: 'dshb-chip' + (p.source === 'extra' ? ' dshb-chip-brand' : ''), children: t(sourceChipKey[p.source]) }), !p.hasKey
                                            ...
                                            : b.ok
                                                ? infos.length === 0
                                                    ...
                                                    : infos.map((info, i) => (_jsxs("div", { children: [_jsxs("div", { className: "dshb-prov-amount", children: [info.total_balance, _jsxs("span", { className: "dshb-prov-sub", children: [" ", info.currency] })] }), _jsxs("div", { className: 'dshb-prov-sub' + (info.topped_out ? ' dshb-topped' : ''), children: [t('balanceGranted'), " ", info.granted_balance, info.topped_out ? ` · ${t('toppedOut')}` : ''] })] }, i)))
    ...
    const costCard = (label, entry) => {
        const official = entry?.official ?? { uncachedInput: 0, cacheRead: 0, cacheWrite: 0, output: 0 };
        ...
        return (_jsxs("div", { className: "dshb-cost-card", children: [_jsx("div", { className: "dshb-cost-label", children: label }), entry === undefined
                    ? _jsx("div", { className: "dshb-spinner" })
                    : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dshb-cost-amount", children: [fmtAmount(entry.amount), _jsx("span", { className: "dshb-cost-currency", children: entry.currency })] }), _jsx("div", { className: "dshb-official-title", children: t('officialLabel') }), officialTotal > 0
                                ? (_jsx("div", { className: "dshb-bar", role: "img", "aria-label": TOKEN_SEGMENTS.map((s) => t(s.labelKey) + ': ' + fmtTokens(official[s.key])).join(', '), children: TOKEN_SEGMENTS.map((seg) => {
                                        const v = official[seg.key];
                                        return v > 0
                                            ? (_jsx("div", { className: "dshb-bar-seg", style: { width: (v / officialTotal * 100) + '%', background: seg.color }, title: t(seg.labelKey) + ': ' + fmtTokens(v) }, seg.key))
                                            : null;
                                    }) }))
                                : _jsx("div", { className: "dshb-bar dshb-bar-empty", children: t('noUsage') }), _jsx("div", { className: "dshb-bar-legend", children: TOKEN_SEGMENTS.map((seg) => (_jsxs("span", { children: [_jsxs("span", { className: "dshb-bar-legend-name", children: [_jsx("i", { style: { background: seg.color } }), t(seg.labelKey)] }), _jsx("b", { children: fmtTokens(official[seg.key]) })] }, seg.key))) }), nonOfficialList.length > 0
                                ? (_jsxs("div", { className: "dshb-nonofficial-bar", children: [_jsx("div", { className: "dshb-nonofficial-title", children: t('nonOfficialLabel') }), nonOfficialList.map((item) => {
                                            const itemTotal = totalTokensView(item.buckets);
                                            if (itemTotal <= 0)
                                                return null;
                                            return (_jsxs("div", { className: "dshb-provider-row", children: [_jsx("span", { className: "dshb-provider-name", children: item.provider }), _jsx("div", { className: "dshb-bar dshb-bar-sm dshb-bar-flex", role: "img", "aria-label": TOKEN_SEGMENTS
                                                            .filter((seg) => item.buckets[seg.key] > 0)
                                                            .map((seg) => t(seg.labelKey) + ': ' + fmtTokens(item.buckets[seg.key]))
                                                            .join(', '), children: TOKEN_SEGMENTS.map((seg) => {
                                                            const v = item.buckets[seg.key];
                                                            return v > 0
                                                                ? (_jsx("div", { className: "dshb-bar-seg", style: { width: (v / itemTotal * 100) + '%', background: seg.color }, title: t(seg.labelKey) + ': ' + fmtTokens(v) }, seg.key))
                                                                : null;
                                                        }) }), _jsx("span", { className: "dshb-provider-counts", children: TOKEN_SEGMENTS
                                                            .filter((seg) => item.buckets[seg.key] > 0)
                                                            .map((seg) => t(seg.labelKey) + ' ' + fmtTokens(item.buckets[seg.key]))
                                                            .join(' · ') })] }, item.provider));
                                        })] }))
    ...
    const renderPricesTab = () => (_jsxs("div", { children: [_jsx("p", { className: "dshb-hint", children: t('pricesHint') }), prices === null
                ...
                : (_jsxs("div", { children: [_jsxs("div", { className: "dshb-window-box", children: [_jsx("div", { className: "dshb-window-title", children: t('windowTitle') }), _jsx("p", { className: "dshb-hint", children: t('windowHint') }), _jsx("div", { className: "dshb-window-list", children: windowCfg.peakWindows.map((w, wi) => (_jsxs("div", { className: "dshb-window-row", children: [_jsx("input", { className: "dshb-input dshb-cur", value: w.start, "aria-label": t('windowStart'), placeholder: "09:00", onChange: (e) => {
                                                    ...
                                                    setWindowCfg({ ...windowCfg, peakWindows: next });
                                                } }), _jsx("button", { type: "button", className: "dshb-btn dshb-btn-small dshb-btn-danger dshb-window-del", onClick: () => setWindowCfg({ ...windowCfg, peakWindows: windowCfg.peakWindows.filter((_, j) => j !== wi) }), children: t('delete') })] }, wi))) }), _jsxs("div", { className: "dshb-window-actions", children: [_jsxs("button", { type: "button", className: "dshb-btn dshb-btn-small", onClick: () => setWindowCfg({ ...windowCfg, peakWindows: [...windowCfg.peakWindows, { start: '09:00', end: '12:00' }] }), children: ["+ ", t('addWindow')] }), _jsxs("label", { className: "dshb-window-tz", children: [_jsx("span", { children: t('tzOffset') }), _jsx("input", { className: "dshb-range", type: "range", min: -12, max: 12, step: 1, value: Math.round(windowCfg.timezoneOffsetMinutes / 60), "aria-label": t('tzOffset'), onChange: (e) => setWindowCfg({ ...windowCfg, timezoneOffsetMinutes: Number(e.target.value) * 60 }) }), _jsx("b", { className: "dshb-window-tz-label", children: formatTimezone(windowCfg.timezoneOffsetMinutes / 60) })] })] })] }), _jsx("div", { className: "dshb-price-scroll", children: _jsxs("table", { className: "dshb-table dshb-price-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { className: "dshb-price-corner", colSpan: 3, children: t('priceModel') }), prices.map((tier, i) => (_jsx("th", { className: "dshb-price-head-cell", children: _jsx("span", { className: "dshb-price-model-name", title: tier.match === '*' ? t('fallbackHint') : undefined, children: tier.name || tier.match }) }, tier.id)))] }) }), _jsx("tbody", { children: METRIC_GROUPS.map((group, gi) => (_jsxs(Fragment, { children: [_jsxs("tr", { children: [gi === 0
                                                            ? _jsxs("th", { rowSpan: METRIC_GROUPS.length * 2, className: "dshb-price-cat", children: [t('priceCategory'), _jsx("sup", { children: "(1)(2)" })] })
                                                            : null, _jsx("th", { rowSpan: 2, className: "dshb-price-metric", children: t(group.labelKey) }), _jsx("td", { className: "dshb-price-period dshb-period-off", children: t('pricePeriodOffPeak') }), prices.map((tier, i) => (_jsx("td", { className: "dshb-price-cell", children: _jsx("input", { className: "dshb-input dshb-num", type: "number", step: "any", min: 0, value: tier.offPeak[group.field], "aria-label": t(group.labelKey) + ' · ' + t('pricePeriodOffPeak'), onChange: (e) => updateRate(i, 'offPeak', group.field, Number(e.target.value) || 0) }) }, tier.id)))] }), _jsxs("tr", { children: [_jsx("td", { className: "dshb-price-period dshb-period-peak", children: t('pricePeriodPeak') }), prices.map((tier, i) => (_jsx("td", { className: "dshb-price-cell", children: _jsx("input", { className: "dshb-input dshb-num", type: "number", step: "any", min: 0, value: tier.peak[group.field], "aria-label": t(group.labelKey) + ' · ' + t('pricePeriodPeak'), onChange: (e) => updateRate(i, 'peak', group.field, Number(e.target.value) || 0) }) }, tier.id)))] })] }, group.field))) })] }) })] })), _jsxs("div", { style: { display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }, children: [_jsx("button", { type: "button", className: "dshb-btn dshb-btn-small", onClick: restoreDefaults, children: t('restoreDefaults') }), _jsx("span", { style: { flex: 1 } }), _jsx("button", { type: "button", className: "dshb-btn dshb-btn-small dshb-btn-primary", disabled: prices === null, onClick: () => { if (prices !== null)
